...
```{r}
# ============================================================
# 2.1 Comparable drift analysis for 2026 calibration data
# ============================================================

library(tidyverse)
library(cowplot)

# Convert your 2026 data to the same conceptual structure
# as Zach's 2024 "raw" data.
cooked <- long_df %>%
  rename(
    id = ID,
    calibration = value,
  ...
  mutate(
    # Absolute proportional deviation from each anchor's
    # calibration on its first available day
    drift = abs(calibration - first(calibration)) /
            first(calibration),

    # Maximum drift observed up to each day
...
# Analogous to Zach's first plot with 2026 data
```{r}
ggplot(
  cooked,
  aes(
  ...
  labs(
    x = "Days",
    y = "Maximum deviation from Day 1"
  ) +
  scale_y_continuous(
...
ggplot(
  ...
  labs(
    x = "Days",
    y = "Instantaneous deviation from Day 1"
  ) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1)
  ) +
...
#instantaneous threshold plot
```{r}
drift.proportion.2 <- crossing(
  cooked,
  threshold = thresholds
) %>%
  group_by(threshold, days) %>%
...
ggplot(
  ...
  theme_minimal()
```
# Day 1 as permanent reference point for 2026 data, so use of ADF box ljung is good
# Normalized verison of the threshold plot
```{r}
drift.proportion_pct <- drift.proportion %>%
  mutate(
    proportion = drifted.above / n_distinct(cooked$id)
  )
```

```{r}
ggplot(
  drift.proportion_pct,
  aes(
    x = days,
    y = proportion,
    colour = threshold,
...
"After 30 days, approximately 70% of anchors had experienced at least one 1% deviation from their initial calibration."

rather than tying the result specifically to a sample of 444 anchors.
